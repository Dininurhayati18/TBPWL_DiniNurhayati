  ...
  <div class="col-md-12">
   <div class="card">
    <div class="card-body">
        ...
        <a href="{{route('print.barang_keluar')}}" target="_blank" class="btn btn-secondary mb-5"><i class="fa fa-print"></i> Print to PDF</a>
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group" roles="group" aria-label="Basic Example">
                            <button type="button" id="btn-delete-bkeluar" class="btn" data-toggle="modal" data-target="#modalDeleteData" data-id="{{ $key->id }}" data-name="{{$key->name}}"><i class="fa fa-trash"></i></button>
                            <button type="button" id="btn-edit-bkeluar" class="btn" data-toggle="modal" data-target="#modalEdit" data-id="{{ $key->id }}" data-name="{{ $key->name }}" data-tanggal="{{ $key->tanggal }}" data-jumlah="{{ $key->jumlah }}"><i class="fa fa-edit"></i></button>
                        </div>
                    </td>
                </tr>
                @endforeach
...
<div class="modal-body">
    <form method="post" action="{{ route('barang_keluar.submit') }}" enctype="multipart/form-data">
        @csrf
...
<div class="modal-body">
    <form method="post" action="{{ route('barang_keluar.update') }}" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <div class="row">
            <div class="col">
...
<div class="modal-body">
    ...
    <form method="post" action="{{ route('barang_keluar.delete') }}" enctype="multipart/form-data">
        @csrf
        @method('DELETE')
</div>
<div class="modal-footer">
...
<script>
    ...
    //    format: 'mm-dd-yyyy'
    //     });
        $(document).on('click', '#btn-edit-bkeluar', function() {
            let id = $(this).data('id');
        ...
        $(document).on('click', '#btn-delete-bkeluar', function() {
            let id = $(this).data('id');
            let name = $(this).data('name');
            $('#delete-id').val(id);
